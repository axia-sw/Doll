...
struct SGfxBinding
{
	...
	SGfxBinding()
		: type(kGfxBindingSampler), location(0), stageBits(0)
	{
		data.sampler = nullptr;
	}
	SGfxBinding(IGfxAPISampler *sampler, U32 location = 0)
		: type(kGfxBindingSampler), location(location), stageBits(0)
	{
		data.sampler = sampler;
	}
	SGfxBinding(IGfxAPITexture *texture, U32 location = 0)
		: type(kGfxBindingTexture), location(location), stageBits(0)
	{
		data.texture = texture;
	}
	SGfxBinding(IGfxAPIUBuffer *ubuffer, U32 location = 0)
		: type(kGfxBindingUniformBuffer), location(location), stageBits(0)
	{
