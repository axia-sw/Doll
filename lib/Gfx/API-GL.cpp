...
public:
	...
	virtual Bool is( const Str &name ) const override {
		return name.caseCmp( "gl" ) || name.caseCmp( "ogl" ) || name.caseCmp( "opengl" );
	}
...
static GLenum getBufferUsage( EBufferPerformance perf, EBufferPurpose purpose ) {
	...
	switch( perf ) {
	case kBufferPerfStream:
		base = GL_STREAM_DRAW;
		break;
	case kBufferPerfStatic:
		base = GL_STATIC_DRAW;
		break;
	case kBufferPerfDynamic:
	...
	GLuint bias = ~0U;
	switch( purpose ) {
	case kBufferPurposeDraw:
		bias = 0;
		break;
	case kBufferPurposeRead:
		bias = 1;
		break;
	case kBufferPurposeCopy:
		bias = 2;
		break;
	}
...
static GLenum getMipFilter( ETextureFilter f, EMipmapMode m ) {
	...
	case kMipmapNone:
		switch( f ) {
		case kTexFilterNearest:
			return GL_NEAREST;
		case kTexFilterLinear:
			return GL_LINEAR;
		}
		break;
...
static GLenum getCompareOp( EGfxCompareOp op ) {
	...
	case kGfxCmpEqual:
		return GL_EQUAL;
	case kGfxCmpLessOrEqual:
		return GL_LEQUAL;
	case kGfxCmpGreater:
		return GL_GREATER;
	case kGfxCmpNotEqual:
		return GL_NOTEQUAL;
	case kGfxCmpGreaterOrEqual:
		return GL_GEQUAL;
	case kGfxCmpAlways:
		return GL_ALWAYS;
	}
...
static GLenum shaderStageToEnum( EShaderStage stage ) {
	...
	case kShaderStageHull:
		return GL_TESS_CONTROL_SHADER;
	case kShaderStageDomain:
...
constexpr size_t shaderTypeToIndex( GLenum shaderType ) {
	switch( shaderType ) {
...
constexpr bool isGraphicsShader( GLenum shaderType ) {
	return shaderTypeToIndex( shaderType ) < shaderTypeToIndex( GL_COMPUTE_SHADER );
}
constexpr bool isComputeShader( GLenum shaderType ) {
	return shaderType == GL_COMPUTE_SHADER;
}

static bool attachShadersToProgram( GLuint programObj, const TArr<IGfxAPIShader *> &shaders, IGfxDiagnostic &diag ) {
	bool hasShader[6]{};
	bool hasAnyGfxShader = false;
	...
	for( const auto &shader : shaders ) {
		...
		const GLuint shaderObj = pointerToObject( shader );

		GLint shaderType = 0;
		glGetShaderiv( shaderObj, GL_SHADER_TYPE, &shaderType );
		CHECKGL();

		const size_t shaderIndex = shaderTypeToIndex( GLenum( shaderType ) );
		if( shaderIndex == ~size_t( 0 ) ) {
			diag.error( Str(), 0, 0, "Invalid shader type" );
			return false;
		}

		if( hasShader[shaderIndex] ) {
			diag.error( Str(), 0, 0, axf( "Program already has %s shader", shaderTypeToString( shaderType ) ) );
			return false;
		}

		hasShader[shaderIndex] = true;
		hasAnyGfxShader |= isGraphicsShader( shaderType );
	}

	const bool hasComputeShader = hasShader[shaderTypeToIndex( GL_COMPUTE_SHADER )];

	if( hasAnyGfxShader && hasComputeShader ) {
		diag.error( Str(), 0, 0, "Compute shaders and graphics shaders are mutually exclusive, but both were bound to the program." );
		return false;
	}

	if( !hasAnyGfxShader && !hasComputeShader ) {
		diag.error( Str(), 0, 0, "No shaders were attached to the program" );
		return false;
	}

	const bool hasVertexShader = hasShader[shaderTypeToIndex( GL_VERTEX_SHADER )];
	const bool hasPixelShader  = hasShader[shaderTypeToIndex( GL_FRAGMENT_SHADER )];

	if( hasAnyGfxShader && !( hasVertexShader && hasPixelShader ) ) {
		diag.error( Str(), 0, 0, "Graphics programs require both a vertex and a pixel shader, but one or more weren't attached." );
		return false;
	}
...
class AutofreeProgram {
	GLuint _program;

public:
	AutofreeProgram( GLuint p )
	: _program( p ) {
	}
	~AutofreeProgram() {
		if( _program != 0 ) {
...
IGfxAPIProgram *CGfxAPI_GL::createProgram( TArr<IGfxAPIShader *> shaders, IGfxDiagnostic *pDiag ) {
	...
	if( !programObj ) {
		diag.error( Str(), 0, 0, "Failed to create program object" );
		return nullptr;
	}

	AutofreeProgram autodeleteProgram( programObj );

	attachShadersToProgram( programObj, shaders, diag );
	glLinkProgram( programObj );

	GLint didLinkSucceed = 0;
	glGetProgramiv( programObj, GL_LINK_STATUS, &didLinkSucceed );

	GLint logSize = 0;
	glGetProgramiv( programObj, GL_INFO_LOG_LENGTH, &logSize );

	MutStr diagString;
	diagString.reserveAndSetLen( axstr_size_t( GLuint( logSize ) ) );
	glGetProgramInfoLog( programObj, GLsizei( diagString.len() ), &logSize, reinterpret_cast<GLchar *>( diagString.data() ) );

	// FIXME: Parse the info log and make separate diagnostics calls

	if( !didLinkSucceed ) {
		if( diagString.isUsed() ) {
			diag.error( Str(), 0, 0, diagString );
		} else {
			diag.error( Str(), 0, 0, "Unknown linking error (GLSL compiler infolog is empty)" );
		}
	...
	if( diagString.isUsed() ) {
		diag.diagnostic( Str(), 0, 0, diagString );
	}

	autodeleteProgram.nvm();
	return objectToPointer<IGfxAPIProgram>( programObj );
}
Void CGfxAPI_GL::destroyShader( IGfxAPIShader *pShader ) {
	...
	const GLuint shaderObj = pointerToObject( pShader );
...
Void CGfxAPI_GL::destroyProgram( IGfxAPIProgram *pProgram ) {
	...
	const GLuint programObj = pointerToObject( pProgram );
	AX_ASSERT_MSG( glIsProgram( programObj ), "Expected program object" );
...
#	endif
		...
		const char *const pszErrorMsg = "OpenGL version is too low.\n\n"
